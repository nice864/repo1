...
*******************************************************************************/
static void ps4_l2cap_connect_ind_cback(BD_ADDR bd_addr, uint16_t l2cap_cid, uint16_t psm, uint8_t l2cap_id) {
  // 修改日誌格式，確保能打印出十六進制的 MAC 地址
  ESP_LOGI(PS4_TAG, "[%s] bd_addr: %02x:%02x:%02x:%02x:%02x:%02x\n  l2cap_cid: 0x%02x\n  psm: %d\n  id: %d", 
           __func__, bd_addr[0], bd_addr[1], bd_addr[2], bd_addr[3], bd_addr[4], bd_addr[5], l2cap_cid, psm, l2cap_id);

  /* 【修改點 1】先對手柄回應 PENDING（等待中狀態），延長底層 ACL 鏈路的加密交握窗口時間 */
  /* 參考了 l2cdefs.h 中的 L2CAP_CONN_PENDING 定義 */
  L2CA_CONNECT_RSP(bd_addr, l2cap_id, l2cap_cid, L2CAP_CONN_PENDING, L2CAP_CONN_PENDING, NULL, NULL);

  /* 【修改點 2】正式發送 OK 回應，確認 L2CAP 通道建立 */
  L2CA_CONNECT_RSP(bd_addr, l2cap_id, l2cap_cid, L2CAP_CONN_OK, L2CAP_CONN_OK, NULL, NULL);

  /* 【修改點 3】完全清空並重置配置結構體，避免殘留原裝手柄的非標準或過大的 MTU 參數，導致第三方手柄拒絕配置 */
  memset(&ps4_cfg_info, 0, sizeof(tL2CAP_CFG_INFO));
  ps4_cfg_info.mtu_present = true;
  ps4_cfg_info.mtu = 650;  // 設為 650。第三方手柄的接收緩衝較小，過大的 MTU 會直接被手柄斷開
